...
(function() {
	...
	_
			.extend(
					...
					Events,
					{
						...
						// A hash of attributes whose current and previous value
						// differ.
						changed : null,
						...
						// The default name for the JSON `id` attribute is
						// `"id"`. MongoDB and
						...
						// Proxy `Backbone.sync` by default -- but override this
						// if you need
						...
						// Returns `true` if the attribute contains a value that
						// is not null
						// or undefined.
						has : function(attr) {
						...
						// Set a hash of model attributes on the object, firing
						// `"change"`. This is
						// the core primitive operation of a model, updating the
						// data and notifying
						...
						set : function(key, val, options) {
							...
							// For each `set` attribute, update or delete the
							// current value.
							for (attr in attrs) {
								val = attrs[attr];
							...
							// You might be wondering why there's a `while` loop
							// here. Changes can
						...
						// `"change"` event.
						// If you specify an attribute name, determine if that
						...
						// Return an object containing all the attributes that
						// have changed, or
						// false if there are no changed attributes. Useful for
						// determining what
						// parts of a view need to be updated and/or what
						// attributes need to be
						// persisted to the server. Unset attributes will be set
						// to undefined.
						// You can also pass an attributes object to diff
						// against the model,
						...
						// Get the previous value of an attribute, recorded at
						// the time the last
						...
						// Get all of the attributes of the model at the time of
						// the previous
						// `"change"` event.
						...
						// Fetch the model from the server. If the server's
						// representation of the
						// model differs from its current attributes, they will
						// be overridden,
						...
						// Set a hash of model attributes, and sync the model to
						// the server.
						// If the server returns an attributes hash that
						// differs, the model's
						...
						save : function(key, val, options) {
							...
							// If we're not waiting and attributes exist, save
							// acts as
							// `set(attr).save(null, opts)` with validation.
							// Otherwise, check if
							// the model will be valid when the attributes, if
							// any, are set.
							...
							// After a successful server-side save, the client
							// is (optionally)
							...
							var success = options.success;
							options.success = function(resp) {
								// Ensure attributes are restored during
								// synchronous saves.
						...
						// Destroy this model on the server if it was already
						// persisted.
						// Optimistically removes the model from its collection,
						// if it has one.
						// If `wait: true` is passed, waits for the server to
						// respond before removal.
						...
						// Default URL for the model's representation on the
						// server -- if you're
						// using Backbone's restful methods, override this to
						// change the endpoint
						// that will be called.
						url : function() {
						...
						// **parse** converts a response into the hash of
						// attributes to be `set` on
						// the model. The default implementation is just to pass
						// the response along.
						parse : function(resp, options) {
							return resp;
						},

						// Create a new model with identical attributes to this
						// one.
						clone : function() {
							return new this.constructor(this.attributes);
						},

						// A model is new if it has never been saved to the
						// server, and lacks an id.
						isNew : function() {
							return this.id == null;
						},
						...
						_validate : function(attrs, options) {
							...
							var error = this.validationError = this.validate(
									attrs, options)
									|| null;
							if (!error)
								return true;
							this.trigger('invalid', this, error, _.extend(
									options, {
	...
	_
			.extend(
					...
					Events,
					{
						...
						// The default model for a collection is just a
						// **Backbone.Model**.
						...
						// Update a collection by `set`-ing a new list of
						// models, adding new ones,
						// removing models that are no longer present, and
						// merging models that
						// already exist in the collection, as necessary.
						// Similar to **Model#set**,
						// the core operation for updating the data contained by
						// the collection.
						set : function(models, options) {
							...
							var singular = !_.isArray(models);
							models = singular ? (models ? [ models ] : []) : _
									.clone(models);
							var i, l, id, model, attrs, existing, sort;
							var at = options.at;
							var targetModel = this.model;
							var sortable = this.comparator && (at == null)
									&& options.sort !== false;
							...
							for (i = 0, l = models.length; i < l; i++) {
								...
								// If a duplicate is found, prevent it from
								// being added and
								...
								if (existing = this.get(id)) {
									...
									// If this is a new, valid model, push it to
									// the `toAdd` list.
								} else if (add) {
							...
							// See if sorting is needed, update `length` and
							// splice in new models.
							...
							// Unless silenced, it's time to fire all
							// appropriate add/sort events.
							if (!options.silent) {
								for (i = 0, l = toAdd.length; i < l; i++) {
						...
						// When you have more items than you want to add or
						// remove individually,
						// you can reset the entire set with a new list of
						// models, without firing
						// any granular `add` or `remove` events. Fires `reset`
						// when finished.
						...
						// Return models with matching attributes. Useful for
						// simple cases of
						// `filter`.
						...
						// need to call this under
						// normal circumstances, as the set will maintain sort
						// order as each item
						// is added.
						...
						// resetting the
						// collection when they arrive. If `reset: true` is
						// passed, the response
						// data will be passed through the `reset` method
						// instead of `set`.
						fetch : function(options) {
							options = options ? _.clone(options) : {};
							if (options.parse === void 0)
							...
							options.success = function(resp) {
								...
								collection.trigger('sync', collection, resp,
										options);
							};
							wrapError(this, options);
							return this.sync('read', this, options);
						...
						// collection immediately, unless `wait: true` is
						// passed, in which case we
						...
						// **parse** converts a response into a list of models
						// to be added to the
						// collection. The default implementation is just to
						// pass it through.
						parse : function(resp, options) {
							return resp;
						},

						// Create a new collection with an identical list of
						// models as this one.
						clone : function() {
							return new this.constructor(this.models);
						},

						// Private method to reset all internal state. Called
						// when the collection
						...
						// Prepare a hash of attributes (or other model) to be
						// added to this
						// collection.
						_prepareModel : function(attrs, options) {
							...
							this.trigger('invalid', this,
									model.validationError, options);
							return false;
						},

						// Internal method to sever a model's ties to a
						// collection.
						...
						// Internal method called every time a model in the set
						// fires an event.
						// Sets need to update their indexes when models change
						// ids. All other
						// events simply proxy through. "add" and "remove"
	...
	_.extend(View.prototype, Events, {
		...
		setElement : function(element, delegate) {
			...
			this.$el = element instanceof Backbone.$ ? element : Backbone
					.$(element);
			this.el = this.$el[0];
			if (delegate !== false)
				this.delegateEvents();
	...
	Backbone.sync = function(method, model, options) {
		...
		if (options.data == null
			...
			params.data = JSON
					.stringify(options.attrs || model.toJSON(options));
		}
	...
	_.extend(Router.prototype, Events, {
		...
		_routeToRegExp : function(route) {
			route = route.replace(escapeRegExp, '\\$&').replace(optionalParam,
					'(?:$1)?').replace(namedParam, function(match, optional) {
	...
	_
			.extend(
					...
					Events,
					{
						...
						// The default interval to poll for hash changes, if
						// necessary, is
						// twenty times a second.
						interval : 50,

						// Gets the true hash value. Cannot use location.hash
						// directly due to bug
						// in Firefox where location.hash will always be
						// decoded.
						...
						// Get the cross-browser normalized URL fragment, either
						// from the URL,
						...
						// Start the hash change handling, returning `true` if
						// the current URL matches
						...
						start : function(options) {
							...
							// Figure out the initial configuration. Do we need
							// an iframe?
							...
							// Normalize root to always include a leading and
							// trailing slash.
							...
							// Depending on whether we're using pushState or
							// hashes, and whether
							// 'onhashchange' is supported, determine how we
							// check the URL state.
							if (this._hasPushState) {
								Backbone.$(window)
							...
							// Determine if we need to change the base url, for
							// a pushState link
							...
							// Transition from hashChange to pushState or vice
							// versa if both are
							// requested.
							if (this._wantsHashChange && this._wantsPushState) {

								// If we've started off with a route from a
								// `pushState`-enabled
								// browser, but we're currently in a browser
								...
								if (!this._hasPushState && !atRoot) {
									...
									this.location.replace(this.root
											...
											+ this.fragment);
									// Return immediately as browser will do
									// redirect to new url
									return true;

									// Or if we've started out with a hash-based
									// route, but we're currently
									// in a browser where it could be
									// `pushState`-based instead...
								} else if (this._hasPushState && atRoot
										&& loc.hash) {
						...
						// Disable Backbone.history, perhaps temporarily. Not
						// useful in a real app,
						...
						// Checks the current URL to see if it has changed, and
						// if it has,
						// calls `loadUrl`, normalizing across the hidden
						// iframe.
						checkUrl : function(e) {
						...
						// Attempt to load the current URL fragment. If a route
						// succeeds with a
						// match, returns `true`. If no defined routes matches
						// the fragment,
						// returns `false`.
						...
						// Save a fragment into the hash history, or replace the
						// URL state if the
						// 'replace' option is passed. You are responsible for
						// properly URL-encoding
						// the fragment in advance.
						//
						// The options object can contain `trigger: true` if you
						// wish to have the
						// route callback be fired (not usually desirable), or
						// `replace: true`, if
						// you wish to modify the current URL without adding an
						...
						navigate : function(fragment, options) {
							...
							// Strip the fragment of the query and hash for
							// matching.
							...
							// If pushState is available, we use it to set the
							// fragment as a real URL.
							if (this._hasPushState) {
								this.history[options.replace ? 'replaceState'
										: 'pushState']({}, document.title, url);

								// If hash changes haven't been explicitly
								// disabled, update the hash
								...
								if (this.iframe
										&& (fragment !== this.getFragment(this
												.getHash(this.iframe)))) {
									// Opening and closing the iframe tricks IE7
									// and earlier to push a
									// history entry on hash-tag change. When
									// replace is true, we don't
									// want this.
									if (!options.replace)
										this.iframe.document.open().close();
									this._updateHash(this.iframe.location,
											fragment, options.replace);
								}

								// If you've told us that you explicitly don't
								// want fallback hashchange-
								// based history, then `navigate` becomes a page
								// refresh.
							} else {
						...
						// Update the hash location, either replacing the
						// current entry, or adding
						...
						_updateHash : function(location, fragment, replace) {
							if (replace) {
								...
								// Some browsers require that `hash` contains a
								// leading #.
